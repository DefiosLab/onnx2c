  ...
  for (int c = 0; c < channels; c++) {
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        ...
        output->data[c * img_size + i * width + j] =
            input->data[c * img_size + i * width + j] > 0
                ? input->data[c * img_size + i * width + j]
                : 0;
      }
    }
  }
...
void Add_F32(float_tensor *A, float_tensor *B, float_tensor *C) {
  ...
  for (int c = 0; c < channels; c++) {
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        C->data[c * img_size + i * width + j] =
            A->data[c * img_size + i * width + j] +
            B->data[c * img_size + i * width + j];
      }
    }
  }
}
...
void BatchNormalization_F32(float_tensor *X, float_tensor *scale,
  ...
  for (int c = 0; c < in_ch; c++) {
    ...
    /* float output_mean = mean->data[c] * momentum + saved_mean * (1 -
     * momentum); */
    ...
    for (int i = 0; i < in_h; i++) {
      for (int j = 0; j < in_w; j++) {
        int32_t idx = c * in_h * in_w + i * in_w + j;
        Y->data[idx] = scale->data[c] * (X->data[idx] - output_mean) /
                           sqrt(output_var + epsilon) +
                       B->data[c];
      }
    }
  }
        ...
        for (int j = 0; j < out_w; j++) {
          ...
          for (int ki = 0; ki < ker_h; ki++) {
            for (int kj = 0; kj < ker_w; kj++) {
              ...
              if (osi >= 0 && osi < in_h && osj >= 0 && osj < in_w) {
                int32_t in_idx = ich * in_h * in_w + osi * in_w + osj;
                int32_t w_idx = och * (in_ch * ker_h * ker_w) +
                                ich * ker_h * ker_w + ki * ker_w + kj;
                output->data[out_idx] +=
                    weight->data[w_idx] * input->data[in_idx];
              }
            }
          }
        }
